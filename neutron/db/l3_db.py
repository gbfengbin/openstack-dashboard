...
class Myflow(model_base.BASEV2):
    fix_ip = sa.Column(
        sa.String(15),primary_key=True)
#     flow_id = sa.Column(sa.String(36), nullable=False)
    ...
    flows = sa.Column(sa.String(255), nullable=True)
...
#     def _get_flows(self, context, router_id):
#         try:
...
#         LOG.info(_("aaaaaaaaaaaaaa%s")%marker_obj)
#         LOG.info(_("aaaaaaaaaaaaaa"))
        re = self._get_collection(context, Router,
...
#     def get_flows(self, context, fix_ip):
#         flows = self._get_flows(context, fix_ip)
#         return self._make_flows_dict(flows)

    def get_flows(self, context, filters=None, fields=None,
                    sorts=None, limit=None, marker=None,
                    page_reverse=False):
        marker_obj = self._get_marker_obj(context, 'flows', limit, marker)
#         LOG.info(_("aaaaaaaaaaaaaa%s")%filters)
        re = self._get_collection(context, Myflow,
                                    self._make_flows_dict,
                                    fields=fields,
                                    sorts=sorts,
                                    limit=limit,
                                    marker_obj=marker_obj,
                                    page_reverse=page_reverse)
        LOG.info(_("flowsdb     :%s")%re)
        return re
    
    def _make_flows_dict(self, flows, fields=None):
        res = {
               'flows': flows['flows'],
               'fix_ip': flows['fix_ip']
               }
    ...
    def _get_flows(self, context, fix_ip):
        try:
            myflows = self._get_by_fix_ip(context, Myflow, fix_ip)
        except exc.NoResultFound:
            raise l3.MyflowsNotFound(floatingip_id=fix_ip)
        return myflows
    ...
    def add_flows(self,context,fix_ip,flows):
...
#                                 flow_id=flow_id,
    ...
    def update_flows(self,context, fix_ip, flows):
#         myflows = self._get_flows(context, fix_ip)
        flow_query = self._model_query(context,Myflow).filter(
                                                              Myflow.fix_ip == fix_ip)
#         LOG.info(_("flowsaaaaaaa%s")%flow_query)
        flow_query.update({'flows':flows},synchronize_session=False)
